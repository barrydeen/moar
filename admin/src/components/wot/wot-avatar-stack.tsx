...
interface WotAvatarStackProps {
  seed: string;
  relays: string[];
}

export function WotAvatarStack({ seed, relays }: WotAvatarStackProps) {
  const { data, isPending } = useWotFollows(seed, relays);
  ...
  const MAX_SHOW = 8;
  const shown = data.contacts.slice(0, MAX_SHOW);
  const remaining = data.contacts.length - MAX_SHOW;

  return (
    <div className="flex items-center mt-2">
      <div className="flex -space-x-2">
        {shown.map((pubkey) => {
          const profile = data.profiles.get(pubkey) ?? { pubkey };
          return <MiniAvatar key={pubkey} profile={profile} />;
        })}
      </div>
      {remaining > 0 && (
